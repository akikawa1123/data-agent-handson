...
1. 右上の「**保存**」をクリック
2. ワークスペースに `TravelSalesModel` が作成されていることを確認

> **ポイント:** セマンティックモデルにリレーションを定義しておくことで、Data Agent が自動的にテーブルを結合して回答を生成できるようになります。
...
### 3-4. Agent の指示（System Instructions）の設定

Agent の振る舞いとデータの内容を定義するインストラクションを設定します。テーブルやカラムの説明もここに含めることで、Data Agent の回答精度が向上します。

1. 画面上部の「**セットアップ**」タブを選択
2. 「**インストラクション**」欄に以下のテキストをコピー＆ペースト:
...
| System Instructions を調整する | 回答フォーマットや使用言語を明示する |
...
**A:** System Instructions に「必ず日本語で回答してください」と明示的に追加してください。また、カラムの説明を日本語で充実させると精度が向上します。
