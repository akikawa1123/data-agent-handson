...
> **📝 グランドトゥルースについて:**  
> `evaluation_questions.csv` に含まれる期待される回答（グランドトゥルース）は、付録の**参考 SQL クエリ集**を Lakehouse の SQL 分析エンドポイントで実行して得られた結果をもとに作成しています。
...
評価 SDK から Data Agent を呼び出すには、Agent を**公開（Publish）**しておく必要があります。

1. `TravelSalesAgent` の画面を開く
2. 画面右上の「**公開**」ボタンをクリック
3. 確認ダイアログが表示されたら「**公開**」をクリック
4. 画面上部に「公開済み」と表示されることを確認

> **注意:** 公開後に Agent の設定（指示・データソース）を変更した場合は、再度「公開」をクリックして反映させてください。
